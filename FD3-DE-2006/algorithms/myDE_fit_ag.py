...
from functions import G22_1
from functions import G13_1
from tools import gradient
from fitness import fit_vio
...
class DE():
    ...
    def init_Population(self):
        ...
        for i in range(self.pN):
            ...
            fitness, res = fit_vio.fit(self.G.f, self.X[i], self.G.fmax, self.G.fmin, self.G.v, self.e)
            self.Xfv[i] = fitness
    ...
    def iterator(self):
        ...
        for p in range(m):
            f = lambda X: self.G.f(X)[p + 1]
            Cfunctions.append(f)
            g = lambda X: -self.G.f(X)[p + 1] if self.G.f(X)[p + 1] > 0 else 0
            deltaCfunctions.append(g)

        for t in range(self.tmax):

            # ee = self.e * ((t + 0.000001) / self.tmax) ** self.cp
            # ee = 0.001
            ee = self.e * (1 - (t / (self.tmax + 0.000001))) ** self.cp
            ...
            for i in range(self.pN):
                ...
                基于梯度的融合
                """
                gra = np.zeros((m, self.dim))
                deltaC = np.zeros((m))

                for q in range(3):
                    if np.random.random() < 0.01:
                        for p in range(m):
                            gra[p] = gradient.numberical_grandient(Cfunctions[p], X_new)
                            deltaC[p] = deltaCfunctions[p](X_new)
                        # print("gra")
                        # print(gra)
                        # print("deltaC")
                        # print(deltaC)
                        # print(X_new)
                        X_new = X_new + np.dot(np.linalg.pinv(gra), np.transpose(deltaC))
                        # print("-------------------------")
                        # print(X_new)
                        # for j in range(self.dim):
                        #     """
                        ...
                        #     """
                        #     if X_new[j] < self.G.bounds[j][0] or X_new[j] > self.G.bounds[j][1]:
                        #         X_new[j] = self.G.bounds[j][0] + (
                        #                     self.G.bounds[j][1] - self.G.bounds[j][0]) * np.random.random()
                ...
                X_new_fitness, res = fit_vio.fit(self.G.f, X_new, self.G.fmax, self.G.fmin, self.G.v, ee)
                ...
                if self.Xbestfv[2] == 0.0:
                    self.fe = 1
                    if self.FES == self.tmax * self.pN and de <= 0.00001:
                ...
                print((self.Xbestfv[0], self.Xbestfv[1], self.Xbestfv[2], t * self.pN + i, self.fe, self.su))
...
if __name__ == "__main__":
    # def __init__(self, pN, dim, tmax, G,  F, cr, cp, e):
    d = DE(40, G22_1.dim, 125000, G22_1, 0.7, 0.9, 5, 0.000001)
    d.init_Population()
    res = d.iterator()
