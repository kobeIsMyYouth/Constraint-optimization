    ...
    def iterator(self):
        ...
        for t in range(self.tmax):
            ...
            for i in range(self.pN):
                ...
                if fit_com.fit(X_new_f, X_new_v, QXfv[i][0], QXfv[i][1], self.epsilon):
                    self.X[i] = X_new
                    self.Xfv[i][0] = X_new_f
                    self.Xfv[i][1] = X_new_v
                    if fit_com.fit(X_new_f, X_new_v, Xbestfv[0], Xbestfv[1], self.epsilon):
                        self.Xbest = X_new
...
if __name__ == "__main__":
    # def __init__(self, pN, dim, tmax, G, Ne, epsilon, tc, F, cr, cp):
    d = DE(40, G10.dim, 10, G10, 3, 0, 2500, 0.7, 0.9, 5)
    d.init_Population()
    res = d.iterator()
    print(res[0])
